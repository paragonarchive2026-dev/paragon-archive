...
(function (global) {
  ...
  function applyTheme() {
    /* P-113 — each site follows the ARCHIVE theme (the switch in the Archive nav bar
       controls every Paragon website). The site no longer has its own day/night toggle. */
    try {
      const archiveTheme = localStorage.getItem("paragonArchive.theme.v2");
      const archiveMode = localStorage.getItem("paragonArchive.themeMode.v2") || "auto";
      let dark = true;
      if (archiveTheme === "light") dark = false;
      else if (archiveTheme === "dark") dark = true;
      else if (archiveMode === "light") dark = false;
      else if (archiveMode === "dark") dark = true;
      else {
        const hour = new Date().getHours();
        dark = !(hour >= 6 && hour < 18); // auto day/night
      }
      document.documentElement.classList.toggle("light", !dark);
      document.documentElement.dataset.themeMode = archiveMode;
    } catch (error) { /* ignore */ }
  }
