...
const LIVE_SITES = new Set([
  ...
  "Paragon Photo",
  "Paragon Shop",
  // P-116 — first built game on the shared games framework (GAMES-BUILD-PLAN.md)
  "Paragon Cards"
]);
...
/* Games live under /games/<slug>/ (GAMES-BUILD-PLAN.md §2), product sites under /sites/<slug>/. */
const IN_PROJECT_ROOTS = ["sites/", "games/", "paragon-quiz/"];
for (const name of LIVE_SITES) {
  if (name === "Paragon Archive Hub") continue;
  const live = catalogue.ParagonSites.find(site => site.name === name);
  assert(live && live.siteUrl && IN_PROJECT_ROOTS.some(root => live.siteUrl.startsWith(root)) && !live.previewOnly, `${name} must open its in-project build (P-099 / P-116)`);
...
assert2(sw.includes("paragon-archive-v90"), "Cache must be v73 (P-096)");
...
assert3(sw.includes("paragon-archive-v90"), "Cache must be v74 (P-097)");
...
assert4(sw.includes("paragon-archive-v90"), "Cache must be v76 (P-099)");
...
assert5(read5("service-worker.js").includes("paragon-archive-v90"), "cache must be v77");
...
a6(r6("service-worker.js").includes("paragon-archive-v90"), "cache must be v83");
...
a7(r7("service-worker.js").includes("paragon-archive-v90"), "cache must be v83");
...
a8(r8("service-worker.js").includes("paragon-archive-v90"), "cache must be v83");
...
a9(r9("service-worker.js").includes("paragon-archive-v90"), "cache must be v83");
...
aA(rA("service-worker.js").includes("paragon-archive-v90"), "cache must be v83");
...
aB(rB("service-worker.js").includes("paragon-archive-v90"), "cache must be v83");
...
aC(rC("service-worker.js").includes("paragon-archive-v90"), "cache must be v84");
...
aD(rD("service-worker.js").includes("paragon-archive-v90"), "cache must be v85");
...
aE(rE("service-worker.js").includes("paragon-archive-v90"), "cache must be v86");
...
aF(rF("service-worker.js").includes("paragon-archive-v90"), "cache must be v87");
...
aG(rG("service-worker.js").includes("paragon-archive-v90"), "cache must be v89");
...
assert3(!fs3.existsSync(path3.join(root3, "exports")), "exports/ take-away cancelled by owner (P-097 build-here decision)");
assert3(read3("supabase/schema.sql").includes("EXECUTED LIVE"), "schema.sql must be labelled the executed archive reference (P-097)");
assert3(sw.includes("paragon-archive-v90"), "Cache must be v89 (union cache)");
console.log("PASS: P-097 — consolidated 30-panel desk with role law + two-way role sync, maintenance lockdowns (platform + per-site), preview window manager, install/permissions popup + share-install, auto day/night, 10 achievement badges, icon-facaded cards, honest review counts, profile editor popup, waste swept");
})();
...
check5(sw5.includes("\"./paragon-leaderboards.js\"") && sw5.includes("paragon-archive-v90"), "service worker precaches the engine at cache v77");
